...
app_server <- function( input, output, session ) {
  ...
  output$headerUtilization <- shiny::renderUI({
    ReportEnd <- 
      lubridate::ceiling_date(input$utilizationDate, unit = "month") - lubridate::days(1)
    ReportStart <- 
      lubridate::floor_date(ReportEnd, unit = "month") - lubridate::years(1) + lubridate::dmonths(1)
    list(
  ...
  output$headerCovid19 <- shiny::renderUI({
    list(
      ...
      shiny::h4(rm_dates()$hc$began_collecting_covid_data, "-", rm_dates()$meta_HUDCSV$Export_End)
    )
  })
  ...
  output$headerCoCCompetitionProjectLevel <- renderUI({
    ...
    next_thing_due <- tibble::tribble(
      ~ DueDate, ~ Event,
      ymd(rm_dates$hc$project_eval_docs_due), "Projects submit program documents to evidence  best practices and CE Prioritization compliance",
