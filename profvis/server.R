...
output$covidPrioritization <- renderPlot({
  ...
  get_res_prior <- validation %>%
    ...
    slice(1L)
  # TODO Some of these transformations are reused 
...
output$covidStatus <- renderPlot({
  ...
  plot
    ...
    renderPlotly({
      ...
      ReportingPeriod <- interval(ymd(ReportStart),
                                  # TODO Duplicate ymd
                                  ymd(ReportEnd))
  ...
  # REVIEW Can we make this non-date dependent so it doesn't have to be manually updated?  
    a <- tribble(
  ...
  output$QPRLoSPlot <-
    ...
    renderPlotly({
      ...
      LoSGoals <- goals %>%
        select(-Measure) %>%
        # TODO This mutate is performed in multiple instances: #1787, #1942, #2100. a great candidate for a reactive.
        mutate(
      ...
      if (nrow(LoSDetail) > 0) {
        ...
        #QUESTION Should there be an informative error message indicating that invalid values were input and how they should be adjusted? It might be confusing to a user if their inputs just don't show anything.
      }
    })
               # })
  ...
  output$QPRNCBs <- renderPlotly({
    ...
    stagingNCBs <- NCBsAtExit %>%
      ...
      # TODO Functionalize this hover as it's used in multiple places. 
      mutate(
        hover = paste0(
          FriendlyProjectName, 
