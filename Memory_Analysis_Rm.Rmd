...
data_objects <- purrr::map(list.files("../COHHIO_HMIS/images", pattern = "R[dD]ata$", full.names = TRUE) %>% setNames(nm = .), ~{
  e <- new.env()
  load(.x, envir = e)
...
files <- c(server = "app_server", u = "app_ui", global = "global")
files <- purrr::map(files, ~{
  lines <- readr::read_lines(fs::path("R", .x, ext = "R"))
})
...
if (stringr::str_detect(getwd(), "Rminor$")) {
  # Remove between functions if Rminor
 .call <- stringr::str_subset(.call, "\\_between$", negate = TRUE) 
}

.call <- paste0(stringr::str_subset(sort(.call), "^\\.", negate = T), collapse = ", ")
