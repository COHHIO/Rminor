...
qpr_dependencies$TPH_youth <- c(
  "qpr_leavers",
  "goals"
)

qpr_expr$temp_permanent_housing_youth <- list()

qpr_expr$temp_permanent_housing_youth$expr <- rlang::expr({
  req(input$date_range, input$region)
...
qpr_expr$temp_permanent_housing_youth$infobox <- rlang::expr({
  req(data_env())
...
qpr_expr$temp_permanent_housing_youth$details <- rlang::expr({
  tibble::tibble(
    ProjectType = c("Street Outreach"),
