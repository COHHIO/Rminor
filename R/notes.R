utilization_notes <- list(bed = "Bed Utilization is the percentage of a project's available beds being populated by individual clients.",
                          ...
                          calc = tagList(tags$ul(tags$li("Bed Utilization = bed nights* served / total possible bed nights** in a month."),
                                                 tags$li("Unit Utilization = unit nights* served / total possible unit nights in a
month.")),
tags$em(
  "* A bed night is a single night in a bed.", tags$br(),
  "** Total possible bed/unit nights = number of beds/units a project has multiplied by how many days are in the given month."
),
tags$ul(
  tags$li("Example A: Client A enters a shelter on May 1 and exits on May 5. They spent four nights in the shelter, so that was 4 bed nights from that client alone in the month of May for that shelter."),
  tags$li("Example B: PSH Project A served 10 people every single night in the month of June. Each client was served 30 bed nights during that month, and since there were 10 clients, that PSH project served a total of 300 bed nights for the month of June."),
  tags$li("Example C: PSH Project B has 5 beds. That project's total possible bed
nights for the month of April (which has 30 days in it) is 30 x 5, which is 150."),
  tags$li("Example D: Using what we know from Example B of PSH Project A's total bed nights for the month of June, let's calculate what their bed utilization was for that month. They have 11 beds and June has 30 days so since 11 × 30 = 330 possible bed nights. Their bed utilization is bed nights (300) divided by possible bed nights (330), which is: 91%!")
