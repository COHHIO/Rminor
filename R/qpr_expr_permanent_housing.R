...
#### Expressions for youth measures
qpr_dependencies$PH_youth <- c(
  "qpr_leavers",
  "goals"
)

qpr_expr$permanent_housing_youth <- list()

qpr_expr$permanent_housing_youth$expr <- rlang::expr({
  req(input$date_range, input$region)
...
qpr_expr$permanent_housing_youth$infobox <- rlang::expr({
  req(data_env())
...
qpr_expr$permanent_housing_youth$details <- rlang::expr({
  tibble::tibble(
    ProjectType = c("Street Outreach", "Emergency Shelter", "Transitional Housing", "Rapid Re-housing"),
